...
### Atacadão no Claude, ChatGPT e agentes de IA via Open Finance

Conecte sua conta do **Atacadão** ao Claude, ChatGPT e agentes de IA via Open Finance Brasil. Pergunte sobre saldos, extratos, faturas de cartão e investimentos do Atacadão em linguagem natural. Regulado pelo Banco Central, somente leitura.

- 🏦 **Atacadão via Open Finance Brasil**, regulado pelo Banco Central
...
**Manual** (se o deeplink não abrir): [claude.ai/customize/connectors](https://claude.ai/customize/connectors) → **+** → **Adicionar conector personalizado** → cole **Nome** `Atacadão MCP` e **URL** `https://api.mcp.ai/p_atacadao`.

### Cursor

[➕ Instalar Atacadão MCP no Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=atacadao&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9hdGFjYWRhbyJ9)

### VS Code (Copilot Chat)

[➕ Instalar Atacadão MCP no VS Code](vscode:mcp/install?name=atacadao&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_atacadao%22%7D)
...
```
Quanto gastei no cartão do Atacadão esse mês?
Resuma minha fatura do Atacadão em aberto
Qual meu saldo na conta do Atacadão?
Como está minha carteira de investimentos no Atacadão?
```
...
**Como funciona?**
Via Open Finance Brasil (regulado pelo Banco Central). Você conecta sua conta Atacadão com consentimento explícito; a IA lê saldos, extratos, faturas e investimentos. Somente leitura.

**A IA pode movimentar dinheiro?**
Não. 100% leitura — Open Finance não permite movimentação por essa via.

**Só funciona com o Atacadão?**
Este MCP é focado no Atacadão. Para conectar vários bancos de uma vez, use o Banco MCP (api.mcp.ai/banco).
...
- 📧 [atacadao@mcp.ai](mailto:atacadao@mcp.ai)
- 🐛 [GitHub Issues](https://github.com/mcp-dir/atacadao-mcp/issues)
- 📄 [docs/](docs/)
...
MIT — veja [LICENSE](LICENSE). O servidor MCP em `api.mcp.ai/p_atacadao` é proprietário (hosted); este repositório (manifestos, docs, skills) é MIT.
